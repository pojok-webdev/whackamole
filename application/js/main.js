...
{	
	...
	init:function (x, y)
	{
		...
		this.addAnimation ("jackpot", [15]);
		this.addAnimation("padi_true",[0,5,4,0,5,4],0);
		this.addAnimation("padi_false",[11,11,12,12,11,12],0);
		// set default one
...
//			this.animationpause(true);
	...
	update : function ()
	{
		if (this.isVisible) {
			...
			// hide the mode after 1/2 sec. Puji: hide before 1/2 sec bro
			if (this.isOut===true) {
...
{	
	...
	update : function ()
	{
		// every 1/2 seconds display moles randomly, it was 500 dude !!
...
var PlayScreen = me.ScreenObject.extend(
{
   ...
   // no need to do somehting else
	...
	padidialog = function(mole){
		$('<div>Di manakah alamat PadiNET ?</div>').dialog({
			buttons:{
				'benar':function(){
